...
\textbf{Unique Selling Point}. While most visualizations online focus on a static set of input values and parameters and showing a single set of trajectories, our visualization can visualize an essentially infinite (thanks to in-browser VAE interpolation) amount of trajectories for varied parameter values. While showing the trajectories (along with phase and amplitude), a 2-D latent space view visualizes hard-to-digest system dynamics. And this whole system can work for multiple different ODEs even as the number of parameters scale up.
...
was able to enable a users to (1)~identify qualitative behavioral regimes without
...
\begin{itemize}
\item We had specific improvements to our visualization after we changed our random distribution of input data to a uniform distribution to make patterns in the latent space more visible. This improvement came was based on feedback received from Taoseef Aziz after our Interim Presentation.

\item We saw the real value of user feedback in our work after specific suggestions like the uniform window suggestion from Taoseef and the limitation of the clicakble Latent Space view suggestion by Aiden.
